...
wire signed [17:0] pre_sub = a_r - b_r;
reg signed [35:0] product_r = 36'sd0;
always @(posedge clk) begin
    a_r <= a;
    b_r <= b;
    c_r <= c;
    product_r <= pre_sub * c_r;
end
...
PADD18 #(
    .AREG(1'b0), .BREG(1'b0), .ADD_SUB(1'b1),
    .PADD_RESET_MODE("SYNC"), .BSEL_MODE(1'b1), .SOREG(1'b0)
) pre_adder (
    .DOUT(pre_sub), .SO(unused_padd_so), .SBO(unused_padd_sbo),
...
MULT18X18 #(
    ...
    .A(pre_sub), .B(c), .SIA(18'd0), .SIB(18'd0),
