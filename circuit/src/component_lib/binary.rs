...
impl Wires<2> {
    ...
    pub fn eq_const(self, const_value: u8) -> Wire {
        Wires {
            wires: [
                self.wires[0].eq_const(const_value % 2),
...
impl Wires<4> {
    ...
    pub fn eq_const(self, const_value: u8) -> Wire {
        ...
        low.eq_const(const_value % 4) & high.eq_const((const_value >> 2) % 4)
    }
}
impl Wires<8> {
    pub fn all_0(self) -> Wire {
    ...
    pub fn eq_const(self, const_value: u8) -> Wire {
        ...
        low.eq_const(const_value % 16) & high.eq_const((const_value >> 4) % 16)
    }
}
...
fn test_mux8_w_selects_each_input() {
    ...
    let (mut circuit, (select, out)) = build_circuit(|| {
        ...
        let values: [Wires<4>; 8] = std::array::from_fn(|i| Wires::<4>::parse_u8(i as u8 + 1));
