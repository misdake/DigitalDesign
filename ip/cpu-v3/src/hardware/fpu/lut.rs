...
//!
//! This module is the single source of truth for the three LUTs.
//! [`render_lut_init_region`] emits the register-file Verilog BSRAM `initial`
//! block from them; the emulator mirror follows in a later step. The arrays are
//! checked into the source as literals so that the emulator path never builds a
//! table at run time. The literals were
...
/// Physical address of the RSQRT table inside mirror B.
pub const MIRROR_B_RSQRT_BASE: usize = 64;
...
pub fn render_lut_init_region() -> String {
    ...
    push_table(&mut output, "mirror_1", MIRROR_B_RSQRT_BASE, &RSQRT_LUT);
    output.push_str("end\n");
...
mod tests {
    ...
    fn register_ram_lut_init_matches_rendered_region() {
        let source = include_str!("cpu_v3_fpu_register_ram.v");
        assert_eq!(
