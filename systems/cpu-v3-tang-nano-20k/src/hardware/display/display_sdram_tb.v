...
reg reset=0, cpu_request_valid=0, cpu_write=0, cpu_read_line=0, cpu_response_ready=0;
...
initial begin
  ...
  repeat(6) @(posedge clk);
  cpu_address=22'h000200; cpu_read_line=1; cpu_request_valid=1;
  while(!cpu_request_ready) @(posedge clk); @(posedge clk); cpu_request_valid=0; cpu_read_line=0;
  ack(3'b011);
  while (!(controller_command_valid && controller_command==3'b101)) @(posedge clk);
