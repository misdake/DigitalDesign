...
use crate::g16::{Device, PhysicalWordAddress, Word};
...
impl Device for BootDmaDevice {
    ...
    fn write(&mut self, _memory: &mut [Word], channel: u8, value: Word) {
        match channel {
            DMA_FLASH_OFFSET_LOW => self.flash_offset = (self.flash_offset & 0xffff_0000) | u32::from(value),
            DMA_FLASH_OFFSET_HIGH => self.flash_offset = (self.flash_offset & 0xffff) | (u32::from(value) << 16),
            DMA_DESTINATION_LOW => self.destination = (self.destination & 0xffff_0000) | u32::from(value),
            DMA_DESTINATION_HIGH => self.destination = (self.destination & 0xffff) | (u32::from(value) << 16),
            DMA_FILE_SIZE_LOW => self.file_size = (self.file_size & 0xffff_0000) | u32::from(value),
            DMA_FILE_SIZE_HIGH => self.file_size = (self.file_size & 0xffff) | (u32::from(value) << 16),
            DMA_MEMORY_SIZE_LOW => self.memory_size = (self.memory_size & 0xffff_0000) | u32::from(value),
            DMA_MEMORY_SIZE_HIGH => self.memory_size = (self.memory_size & 0xffff) | (u32::from(value) << 16),
