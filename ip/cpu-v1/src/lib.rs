...
extern crate digital_design_circuit;
use digital_design_circuit::{build_circuit, Circuit};
pub(crate) use digital_design_circuit::{
    reg, reg_w, CircuitComponent as CpuComponent, CircuitComponentEmu as CpuComponentEmu,
    EmulatedComponent as CpuComponentEmuContext, Reg as CircuitReg, Regs, Wires,
};
#[allow(unused)]
pub struct CpuV1State {
    pub inst_src: [Instruction; 256],
    pub inst: [Wires<8>; 256],
    pub pc: Regs<8>,       // write in CpuV1
    pub reg: [Regs<4>; 4], // write in RegWrite
    pub mem: [Regs<4>; 256],
    pub mem_page: Regs<4>,
    pub flag_p: CircuitReg,  // write in CpuV1
    pub flag_nz: CircuitReg, // write in CpuV1
    pub flag_n: CircuitReg,  // write in CpuV1
    pub bus_addr0: Regs<4>,
    pub bus_addr1: Regs<4>,
    pub devices: SharedDeviceBus,
}
impl CpuV1State {
    pub fn create(inst_src: [Instruction; 256]) -> Self {
        Self::create_with_bus(inst_src, null_device_bus())
    }

    pub fn create_with_bus(inst_src: [Instruction; 256], devices: SharedDeviceBus) -> Self {
...
pub struct CpuV1StateInternal {
    pub decoder_in: CpuDecoderInput,
    pub branch_in: CpuBranchInput,
    pub next_pc_in: CpuPcInput,
    pub next_pc_out: CpuPcOutput,
}
...
pub struct CpuV1Instance;
pub struct CpuV1MixInstance;
pub struct CpuV1EmuInstance;
...
#[allow(unused)]
pub fn cpu_v1_build(inst_rom: [Instruction; 256]) -> (Circuit, CpuV1State, CpuV1StateInternal) {
    cpu_v1_build_with_bus(inst_rom, null_device_bus())
}

pub fn cpu_v1_build_with_bus(
    inst_rom: [Instruction; 256],
    devices: SharedDeviceBus,
) -> (Circuit, CpuV1State, CpuV1StateInternal) {
    let (circuit, (state1, internal1)) = build_circuit(|| {
        let mut state1 = CpuV1State::create_with_bus(inst_rom, devices);
        let internal1 = CpuV1Instance::build(&mut state1);
        (state1, internal1)
    });
...
#[allow(unused)]
pub fn cpu_v1_build_mix(inst_rom: [Instruction; 256]) -> (Circuit, CpuV1State, CpuV1StateInternal) {
    cpu_v1_build_mix_with_bus(inst_rom, null_device_bus())
}

pub fn cpu_v1_build_mix_with_bus(
    inst_rom: [Instruction; 256],
    devices: SharedDeviceBus,
) -> (Circuit, CpuV1State, CpuV1StateInternal) {
    let (circuit, (state1, internal1)) = build_circuit(|| {
