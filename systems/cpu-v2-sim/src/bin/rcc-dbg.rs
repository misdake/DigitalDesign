...
use cpu_v2_sim::debugger::DebugSession;
...
mod tests {
    ...
    fn playground_no_opt_keeps_scalar_locals_readable() {
        ...
        assert!(
            matches!(session.var_value(&a), cpu_v2_sim::debugger::VarValue::Mem(_, ref words) if words == &[7])
        );
        assert!(
            matches!(session.var_value(&b), cpu_v2_sim::debugger::VarValue::Mem(_, ref words) if words == &[9])
        );
    }
