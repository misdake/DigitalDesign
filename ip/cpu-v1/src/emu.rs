...
use crate::device::{null_device_bus, SharedDeviceBus};
...
impl EmuEnv {
    pub fn new(inst: [Instruction; 256]) -> EmuEnv {
        Self::new_with_bus(inst, null_device_bus())
    }

    pub fn new_with_bus(inst: [Instruction; 256], device: SharedDeviceBus) -> EmuEnv {
        Self {
            inst,
