...
use crate::{BinOp, CmpRhs, CompareOp, Compiler, FuncBuilder, Opts};
...
fn test_loop_sum() {
    ...
    m.while_loop(
        |b| {
            let i = b.get(i);
            b.cmp(i, CmpRhs::Imm(n), CompareOp::Less)
        },
        |b| {
            let s = b.get(sum);
            let i0 = b.get(i);
...
fn test_if_else_max() {
    ...
    let cmp = {
        ...
        f.cmp(a, CmpRhs::Reg(b), CompareOp::Greater)
    };
    let r = f.new_var();
    f.if_else(
        cmp,
...
fn test_break_continue() {
    ...
    m.while_loop(
        |b| {
            let i = b.get(i);
            b.cmp(i, CmpRhs::Imm(20), CompareOp::Less)
        },
        |b| {
            let i0 = b.get(i);
            ...
            // if i == 6 { continue; }
            let i1c = b.get(i);
            let cmp = b.cmp(i1c, CmpRhs::Imm(6), CompareOp::Equal);
            b.if_then(cmp, |b| b.continue_());
            // if sum > 30 { break; }
            let s = b.get(sum);
            let cmp = b.cmp(s, CmpRhs::Imm(30), CompareOp::Greater);
