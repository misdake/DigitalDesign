...
pub enum VarLoc {
    ...
    /// ABI register (params)
    Param(u8),
    /// Source parameter index before a target ABI assigns a register.
    ParamIndex(u8),
...
fn loc_kind(loc: &VarLoc) -> String {
    match loc {
        ...
        VarLoc::ParamIndex(i) => format!("param#{i}"),
...
fn unwrap_global(loc: &VarLoc) -> u16 {
    match loc {
        VarLoc::Global(a) => *a,
        VarLoc::ParamIndex(_) => panic!("frontend parameter location was not lowered"),
        _ => 0,
    }
}
...
fn parse_loc(s: &str) -> Option<VarLoc> {
    ...
    if let Some(n) = s.strip_prefix("param#") {
        return Some(VarLoc::ParamIndex(n.parse().ok()?));
    }
    if s == "ssa" {
