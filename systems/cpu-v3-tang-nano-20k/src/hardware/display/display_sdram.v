module DisplaySdramPort (
    ...
    input wire cpu_request_valid, input wire cpu_write,
    ...
    output reg [15:0] cpu_read_data = 0, output reg cpu_error = 0,
...
reg owner_display = 0, pending_write = 0, prefer_display = 1;
...
always @(posedge clk) begin
    ...
    if (reset || !controller_init_done) begin
        state <= ST_WAIT; cpu_response_valid <= 0; cpu_error <= 0;
        ...
        ST_OP_WAIT: begin
            ...
            if (pending_write && controller_command_ack) begin
                cpu_read_data <= 0; cpu_response_valid <= 1; state <= ST_CPU_RESPONSE;
            end else if (!pending_write && controller_read_valid) begin
                if (owner_display) begin
                    ...
                    cpu_read_data <= pending_address[0] ? controller_read_data[31:16] : controller_read_data[15:0];
                ...
                if (owner_display) display_error <= 1; else begin cpu_error <= 1; cpu_response_valid <= 1; end
                state <= owner_display ? ST_ERROR : ST_CPU_RESPONSE;
            end else timeout_count <= timeout_count + 1'b1;
        end
        ST_CPU_RESPONSE: if (cpu_response_ready) begin
            cpu_response_valid <= 0; cpu_error <= 0; recovery_count <= 0; state <= ST_RECOVERY;
        end
        ST_RECOVERY: if (recovery_count == 3) state <= refresh_due ? ST_REFRESH_REQ : ST_IDLE;
