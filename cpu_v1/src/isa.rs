...
impl InstEncoded {
    fn to_string(self) -> String {
        match self {
            ...
            InstEncoded::Op0(name, _, _) => name.to_string(),
        }
    }
    fn to_binary(self) -> InstBinaryType {
        match self {
            InstEncoded::Op2(_, opcode4, (reg1, reg0)) => {
                (opcode4 << 4) | ((reg1 as u8) << 2) | (reg0 as u8)
            }
            InstEncoded::Op1(_, opcode6, reg0) => (opcode6 << 2) | (reg0 as u8),
            InstEncoded::Op0i4(_, opcode4, imm4) => (opcode4 << 4) | imm4,
            InstEncoded::Op0i3(_, opcode5, imm3) => (opcode5 << 3) | imm3,
