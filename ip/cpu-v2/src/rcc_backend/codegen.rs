...
use super::{REG_RA, REG_SP, REG_TMP};
use crate::isa::*;
use rcc::*;
use std::collections::{HashMap, HashSet};

fn v2_condition(condition: CompareOp) -> Cond {
    match condition {
        CompareOp::Never => Cond::Never,
        CompareOp::Greater => Cond::Greater,
        CompareOp::Equal => Cond::Equal,
        CompareOp::Less => Cond::Less,
        CompareOp::GreaterEqual => Cond::GreaterEqual,
        CompareOp::NotEqual => Cond::NotEqual,
        CompareOp::LessEqual => Cond::LessEqual,
        CompareOp::Always => Cond::Always,
    }
}
...
pub(crate) enum MachineLine {
    ...
    DirectCall {
        func: crate::rcc_backend::FuncName,
        id: usize,
...
pub(crate) struct CallSite {
    pub caller: crate::rcc_backend::FuncName,
    pub id: usize,
}

pub(crate) fn line_size(
    line: &MachineLine,
    caller: crate::rcc_backend::FuncName,
...
pub struct EmittedFunc {
    pub(crate) name: crate::rcc_backend::FuncName,
...
pub fn compile_function(
    ...
    alloc: &Allocation,
    stack_init: u16,
    function_table: &HashMap<crate::rcc_backend::FuncName, u8>,
            ...
            Some(Terminator::Br {
                ...
                if if_true == if_false && next == Some(if_true) {
                    ...
                    lines.push(MachineLine::Branch {
                        cond: v2_condition(cmp.cond),
                        target: *if_true,
                        line: block.term_line,
                    });
                } else if next == Some(if_true) {
                    lines.push(MachineLine::Branch {
                        cond: v2_condition(cmp.cond.invert()),
                        target: *if_false,
                        line: block.term_line,
                    });
                } else {
...
struct CallLowering<'a> {
    caller: crate::rcc_backend::FuncName,
    hot_block: bool,
    function_table: &'a HashMap<crate::rcc_backend::FuncName, u8>,
}
    ...
    match inst {
        Instr::Bin { dst, op, lhs, rhs } => {
            let f = match op {
                ...
                BinOp::Xor => xor,
                BinOp::Mul => {
                    unreachable!("the v2 driver rewrites integer multiply into a mul_16x16 call")
                }
            };
