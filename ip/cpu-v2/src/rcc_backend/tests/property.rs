...
use crate::{simulate, BoolExpr, CmpRhs, CompareOp, Compiler, FuncBuilder, Opts, VarId};
...
fn emit_op(b: &mut FuncBuilder, op: &Op, vars: &[VarId]) {
    match op {
        ...
        Op::IfLess { lhs, imm, body } => {
            ...
            let cmp = b.cmp(x, CmpRhs::Imm(*imm), CompareOp::Less);
        ...
        Op::Repeat { times, body } => {
            ...
            b.while_bool(
                |b| {
                    let c = b.get(counter);
                    BoolExpr::Cmp(b.cmp(c, CmpRhs::Imm(*times), CompareOp::Less))
                },
                |b| {
                    emit_op(b, body, vars);
                    let c = b.get(counter);
