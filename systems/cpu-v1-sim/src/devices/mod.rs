...
use cpu_v1::{DeviceBus, DeviceReadResult};
...
pub fn test_device_full(
    ...
    let (mut circuit, state, _internal) =
        cpu_v1_build_with_bus(inst_rom, shared_device_bus(Devices::new()));
    ...
    if let Some(reg_ref) = reg_ref {
        for (actual, expected) in state.reg.iter().zip(reg_ref) {
            assert_eq!(actual.out.get_u8(&circuit), expected);
        }
    }
    if let Some(mem_ref) = mem_ref {
        for (actual, expected) in state.mem.iter().zip(mem_ref) {
            assert_eq!(actual.out.get_u8(&circuit), expected);
        }
    }
}
